...
def ImageTNormal(图片: np.ndarray, 启用高度: bool) -> np.ndarray:
    """图片转法线纹理
    Args:
        图片 (多维数组): ReadImage方法读取的灰度数组
        启用高度 (布尔): 光影加载时是否显示方块高度
    Returns:
        多维数组: RGBA格式数组图片，可以使用SaveImage方法保存
    """
    灰度矩阵 = 图片转灰度矩阵(图片)
    灰度向量矩阵 = 灰度矩阵转向量矩阵(灰度矩阵)
    R通道法线, G通道法线 = 向量矩阵转法线纹理(图片, 灰度向量矩阵)
    R通道图片 = 通道图片修复(_64位矩阵转图片(R通道法线, channels_output='single'))
    G通道图片 = np.rot90(通道图片修复(_64位矩阵转图片(G通道法线, channels_output='single')), k=1)
    return 合并通道(R通道图片, G通道图片, np.repeat(np.repeat(图片, 8, axis=0), 8, axis=1), 启用高度)
...
def 通道图片修复(image_array):
    ...
    return output_array
def 合并通道(img_r_numpy, img_g_numpy, img_alpha_rgb_numpy, 启用高度):
    img_alpha_gray = 255 if not 启用高度 else 图片转灰度矩阵(img_alpha_rgb_numpy)
