...
<img width="1282" height="751" alt="image" src="https://github.com/user-attachments/assets/3bace48c-a30d-4208-95eb-c598bc16fcd9" /><br/>
...
- 每个文件最多项的数值
#### 输出路径
- 字面意思，输出路径，选择一个输出文件夹，Json文件 会生成在此路径下
##### 输出名称
字面意思，控制 Json文件 的名称，不填会变成滚木
<br/><br/>
### 视频字幕生成
#### 语音生成API配置
##### API URL
...
- 描述：束数 控制质量，值为 7 需要 16G 显卡内存
#### 导出设置
##### 输出路径
- 字面意思，输出路径，选择一个输出文件夹，SRT文件 与 WAV文件 会生成在此路径下
##### 输出名称
...
- 一次生成 JSON文件 中值的数量
- 1 为字幕百分比与音频对其
- 2 为 1 的加速方法（最大加速55%，最低加速30%），
- 测试：<br/>
1.空隙纠错量 [3, 5]<br/>
1.错误率 [5.9%, 7.2%]（二代索引）<br/>
2.空隙纠错量 [4]<br/>
2.错误率 [2.4%]（三代索引）<br/>
##### 空隙纠错量
