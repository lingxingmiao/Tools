...
def 翻译语言列表(
    ...
    llm_contexts: bool = True, llm_contexts_length: int = 6,
    ...
    if 文本文件:
        ...
        向量索引 = faiss.IndexHNSWSQ(向量文件.shape[1], faiss.ScalarQuantizer.QT_fp16, 80)
    ...
    返回列表 = []
    system_prompt = f"/no_thinking 1.仅输出原文翻译内容，不得包含解释、参考、键、额外内容(如“以下是翻译：”或“翻译如下：”等),即使是单个符号也要翻译(遇到&或§需要保留后面一个符号),注意不需要翻译上文,也不要额外解释\n2.返回结构为一个输出内容一个换行,输出行数必须与输入一致,中间不能有多余的空行\n3.返回的译文数量与输出格式必须一致\n4.翻译为{language}语言\n5.翻译领域为Minecraft游戏"
...
def 应用FTB翻译(index: list, mode: str):
...
def 翻译FTB任务(
    ...
    with mp.Pool(processes=int(mp.cpu_count() / 2 if mp.cpu_count() / 2 > 1 else 1)) as 解释器:
        任务结果 = 解释器.imap(读取单个Snbt文件, snbt文件)
    ...
    with mp.Pool(processes=1) as 解释器:
        任务结果 = 解释器.imap(partial(应用FTB翻译, mode=("H" if os.path.isdir(os.path.join(path, "quests")) else "L")), 翻译列表)
...
if __name__ == "__main__":
    ...
    TranslatorLang.add_argument("--llm-contexts", action="store_true", help="是否启用上下文")
    ...
    TranslatorPack.add_argument("--llm-contexts", action="store_true", help="是否启用上下文")
    ...
    ExportJsonl = 解释器1.add_parser("ExportJsonl", help="导出 JSONL 数据集文件")
    ...
    TranslatorFTBQ = 解释器1.add_parser("TranslatorFTBQ", help="翻译 FTB任务 文件夹")
    ...
    TranslatorFTBQ.add_argument("--llm-contexts", action="store_true", help="是否启用上下文")
