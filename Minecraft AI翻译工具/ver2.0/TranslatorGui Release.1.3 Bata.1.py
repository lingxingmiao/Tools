...
class TranslatorGUI:
    def __init__(self, root):
        ...
        if self.api设置框架 is not None:
            ...
            # Natural Language Processing API 容器
            ...
            # 修改：将Other容器移到左侧框架底部
            self.other容器 = ttk.LabelFrame(左侧框架, text="Other")
            self.other容器.pack(fill='both', expand=True, padx=0, pady=(5, 0))
            
            # 添加缓存路径输入框和浏览按钮
            self._创建缓存路径行(self.other容器)
            
            # 新增：Index容器（现在放在右侧框架）
            self.index容器 = ttk.LabelFrame(右侧框架, text="Index")
            self.index容器.pack(fill='both', expand=True, padx=0, pady=(0, 5))
            ...
            # 新增：添加索引模式下拉框
            self._创建索引模式行(self.index容器)
            
            # 新增：添加HNSW参数行（仅在HNSWSQ模式下显示）
            self._创建_hnsw参数行(self.index容器)
            
            # 新增：添加RefineFlat参数行（仅在RefineFlat模式下显示）
            self._创建_refineflat参数行(self.index容器)
    ...
    def _创建索引模式行(self, parent):
        """创建索引模式下拉框"""
        frame = ttk.Frame(parent)
        frame.pack(fill='x', padx=10, pady=5)
        
        label = ttk.Label(frame, text="索引模式:")
        label.pack(side='left')
        
        index_mode_var = tk.StringVar(value="HNSWSQ")  # 默认值设为HNSWSQ
        index_mode_combobox = ttk.Combobox(
            frame,
            textvariable=index_mode_var,
            values=["HNSWSQ", "RefineFlat"],
            state="readonly",
            width=15
        )
        index_mode_combobox.pack(side='left', padx=(10, 0))
        
        # 绑定事件处理
        index_mode_combobox.bind('<<ComboboxSelected>>', self._on_index_mode_change)
        
        setattr(self, "index_mode_var", index_mode_var)
        setattr(self, "index_mode_combobox", index_mode_combobox)
        setattr(self, "hnsw_param_frame", frame)  # 临时占位，后续替换
    
    def _创建_hnsw参数行(self, parent):
        """创建HNSW参数输入控件（ef_construction、M、ef_search）"""
        self.hnsw_param_frame = ttk.Frame(parent)
        self.hnsw_param_frame.pack(fill='x', padx=10, pady=5)
        
        # ef_construction
        ef_label = ttk.Label(self.hnsw_param_frame, text="ef_construction:")
        ef_label.pack(side='left')
        ef_var = tk.IntVar(value=720)
        ef_spinbox = ttk.Spinbox(self.hnsw_param_frame, from_=1, to=1000, increment=1, textvariable=ef_var, format="%.0f", width=8)
        ef_spinbox.pack(side='left', padx=(5, 10))
        setattr(self, "hnsw_ef_construction_var", ef_var)
        
        # M
        m_label = ttk.Label(self.hnsw_param_frame, text="M:")
        m_label.pack(side='left')
        m_var = tk.IntVar(value=128)
        m_spinbox = ttk.Spinbox(self.hnsw_param_frame, from_=1, to=100, increment=1, textvariable=m_var, format="%.0f", width=8)
        m_spinbox.pack(side='left', padx=(5, 10))
        setattr(self, "hnsw_m_var", m_var)
        
        # ef_search
        ef_search_label = ttk.Label(self.hnsw_param_frame, text="ef_search:")
        ef_search_label.pack(side='left')
        ef_search_var = tk.IntVar(value=480)
        ef_search_spinbox = ttk.Spinbox(self.hnsw_param_frame, from_=1, to=1000, increment=1, textvariable=ef_search_var, format="%.0f", width=8)
        ef_search_spinbox.pack(side='left', padx=(5, 0))
        setattr(self, "hnsw_ef_search_var", ef_search_var)
        
        # 初始状态根据默认模式设置
        self._update_index_param_visibility()
    
    def _创建_refineflat参数行(self, parent):
        """创建RefineFlat参数输入控件（k_factor）"""
        self.refineflat_param_frame = ttk.Frame(parent)
        self.refineflat_param_frame.pack(fill='x', padx=10, pady=5)
        
        # k_factor
        k_factor_label = ttk.Label(self.refineflat_param_frame, text="k_factor:")
        k_factor_label.pack(side='left')
        k_factor_var = tk.DoubleVar(value=2.0)
        k_factor_spinbox = ttk.Spinbox(self.refineflat_param_frame, from_=1.0, to=100.0, increment=0.1, textvariable=k_factor_var, format="%.1f", width=8)
        k_factor_spinbox.pack(side='left', padx=(10, 0))
        setattr(self, "refineflat_k_factor_var", k_factor_var)
        
        # 初始隐藏
        self.refineflat_param_frame.pack_forget()

    def _on_index_mode_change(self, event=None):
        """索引模式变化时更新参数可见性"""
        self._update_index_param_visibility()
    
    def _update_index_param_visibility(self):
        """根据索引模式显示/隐藏对应的参数区域"""
        mode = self.index_mode_var.get()
        if mode == "HNSWSQ":
            if hasattr(self, 'hnsw_param_frame'):
                self.hnsw_param_frame.pack(fill='x', padx=10, pady=5)
            if hasattr(self, 'refineflat_param_frame'):
                self.refineflat_param_frame.pack_forget()
        elif mode == "RefineFlat":
            if hasattr(self, 'hnsw_param_frame'):
                self.hnsw_param_frame.pack_forget()
            if hasattr(self, 'refineflat_param_frame'):
                self.refineflat_param_frame.pack(fill='x', padx=10, pady=5)
    ...
    def _获取_current_config(self):
        ...
        config = {
            ...
            "index_mode": self.index_mode_var.get(),
            "hnsw_ef_construction": self.hnsw_ef_construction_var.get(),
            "hnsw_m": self.hnsw_m_var.get(),
            "hnsw_ef_search": self.hnsw_ef_search_var.get(),
            "refineflat_k_factor": self.refineflat_k_factor_var.get(),
    ...
    def _保存配置(self):
        ...
        # 添加或更新当前配置
        all_configs[config_name] = self._获取_current_config()
    ...
    def _加载选中配置(self):
        ...
        try:
            ...
            self.index_mode_var.set(config.get("index_mode", "HNSWSQ"))
            self.hnsw_ef_construction_var.set(config.get("hnsw_ef_construction", 200))
            self.hnsw_m_var.set(config.get("hnsw_m", 16))
            self.hnsw_ef_search_var.set(config.get("hnsw_ef_search", 100))
            self.refineflat_k_factor_var.set(config.get("refineflat_k_factor", 10))
            # 触发界面更新
            self._update_index_param_visibility()
    ...
    def _execute_translation_task(self, task_name):
        ...
        try:
            ...
            # 获取当前配置
            config = self._获取_current_config()
            
            TranslatorLib.Config.LLM_API_URL = config['llm_url']
            TranslatorLib.Config.LLM_API_KEY = config['llm_key']
            TranslatorLib.Config.LLM_API_MODEL = config['llm_model']
            TranslatorLib.Config.LLM_PROMPT_LOCATION = config['llm_prompt_position']
            TranslatorLib.Config.LLM_CONTEXTS = config['llm_context_enabled']
            TranslatorLib.Config.LLM_CONTEXTS_LENGTH = config['llm_context_length']
            TranslatorLib.Config.LLM_MAX_WORKERS = config['llm_concurrency']
            TranslatorLib.Config.LLM_MAX_BATCH = config['llm_batch_size']
            TranslatorLib.Config.LLM_TOP_K = config['llm_top_k']
            TranslatorLib.Config.LLM_TOP_P = config['llm_top_p']
            TranslatorLib.Config.LLM_TEMP = config['llm_temperature']
            
            TranslatorLib.Config.EMB_API_URL = config['embedding_url']
            TranslatorLib.Config.EMB_API_KEY = config['embedding_key']
            TranslatorLib.Config.EMB_API_MODEL = config['embedding_model']
            TranslatorLib.Config.EMB_MAX_TOKENS = config['embedding_tokens']
            TranslatorLib.Config.EMB_MAX_WORKERS = config['embedding_concurrency']
            
            TranslatorLib.Config.VEC_FILE_NAME = config['vector_name']
            TranslatorLib.Config.VEC_FILE_PATH = config['vector_path']
            TranslatorLib.Config.VEC_QUANTIZATION = config['vector_quantization']
            TranslatorLib.Config.VEC_QUANTIZATION_BLOCK_SIZE = config['vector_block_size']
            
            TranslatorLib.Config.INDEX_K = config['index_count']
            
            TranslatorLib.Config.PATH_CACHE = config['cache_path']
            
            TranslatorLib.Config.LANGUAGE_INPUT = self.原始语言代码变量.get()
            TranslatorLib.Config.LANGUAGE_OUTPUT = self.目标语言代码变量.get()
            ...
            if "翻译语言文件" in task_name:
                ...
                TranslatorLib.Translator.翻译语言文件(**kwargs)
                ...
                if file1:
                    kwargs['file1'] = file1
                TranslatorLib.Translator.翻译资源文件(**kwargs)
            elif "翻译FTBQ任务" in task_name:
                kwargs = {
                    'path': path0,
                }
                kwargs = {k: v for k, v in kwargs.items() if v != ""}
                TranslatorLib.Translator.翻译FTB任务(**kwargs)
            elif "翻译BQ任务" in task_name:
                kwargs = {
                    'path': path0,
                }
                kwargs = {k: v for k, v in kwargs.items() if v != ""}
                TranslatorLib.Translator.翻译BQ任务(**kwargs)
                ...
                kwargs = {k: v for k, v in kwargs.items() if v != ""}
                TranslatorLib.Translator.导出数据集(**kwargs)
                ...
                kwargs = {k: v for k, v in kwargs.items() if v != ""}
                TranslatorLib.Translator.导入参考词(**kwargs)
            elif "导入Dict_Mini" in task_name:
                kwargs = {
                    'file': file0,
                }
                kwargs = {k: v for k, v in kwargs.items() if v != ""}
                TranslatorLib.Translator.导入DictMini参考词(**kwargs)
