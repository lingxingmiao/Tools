...
- 格式：<提示词名称>——<嵌入模型名称>———<检索增强生成模式>.npy   <.npy文件大小>。
- .npy文件维度大小会影响输出质量。
- 如果没有发现与嵌入模型匹配或检索增强生成模式匹配的文件会重新以 16并行（Min.4 Max.1024） 重新生成。
10. ##### 检索增强生成模式
- 共有 EM-1P 和 EM-2P 两种模式；
- EM-1P 适用于翻译模型上下文长度不足使用；
- EM-2P 适用于翻译模型上下文长度充足，可以一个键多个值（多种解释）。
...
- 翻译完成的 zh_cn.lang 会与新的 en_us.lang 进行排序，以便维护；
- 翻译完成后会直接覆盖写入 旧版本文件（模组内容以及语言文件为新版本）。
